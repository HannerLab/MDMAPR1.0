...
#    This file is part of MDMAPR 1.0.

#    GPL license notice

#    MDMAPR is free software: you can redistribute it and/or modify
#    it under the terms of the GNU General Public License as published by
#    the Free Software Foundation, either version 3 of the License, or
#    (at your option) any later version.

#    MDMAPR is distributed in the hope that it will be useful,
#    but WITHOUT ANY WARRANTY; without even the implied warranty of
#    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
#    GNU General Public License for more details.

#    You should have received a copy of the GNU General Public License
#    along with MDMAPR.  If not, see <https://www.gnu.org/licenses/>.
...
shinyServer(function(session, input, output) {
    ...
    # In MDMAPR, the data integration works by first transforming the matrix containing \n
    ...
    # of 40 - 85) for one sample. The transformed column will then be combined \n
    # with the metadata in which each row is for one individual sample.  
    
    # The MDMAPR accepts qPCR data from multiple qPCR Machines including:
    # MIC 
    # Biomeme39
    # Biomeme23
    ...
    input_integration <- reactive({
        ...
        {return(NULL)} else {
            ...
            if (input$platform == "MIC") {
                ...
                #the calculated Ct values may go extreme. Here I am assigning those Ct values to 100. This value will be regarded as negative results. 
               qpcr.convert.mic$ct_value[qpcr.convert.mic$ct_value<0] <- 100
               #qpcr.convert.mic$ct_value[qpcr.convert.mic$ct_value > input$thr] <- 100

                # Combine the transformed qPCR data with the metadata 
                metadata.mic <- cbind(metadata.mic, qpcr.convert.mic)
                
                # This integrated datatable will then be displayed on Page 2 - New Data of MDMAPR. 
                metadata.mic
            ...
            ### Machine Option 2: Biomeme three9 ###
                ...
                #qpcr.string.biomeme39$ct_value[qpcr.string.biomeme39$ct_value > input$thr] <- 100
            ...
            ### Machine Option 3: Biomeme Biomeme two3 ###
                ...
                #qpcr.string.biomeme23$ct_value[qpcr.string.biomeme23$ct_value>input$thr] <- 100
    ...
    output$downloadData <- downloadHandler(
        filename = function() {
            paste("MDMAPR", Sys.Date(), ".xlsx", sep="")
        },
    ...
    ct_cut <- reactive({
        if (is.null(input$file1)) {
            ...
            cut <- cut(ct_value, 
                       c(0,20,30,35,40,100),
                       include.lowest = TRUE,
                       labels = c('Very Strong', 'Strong', 'Moderate', 'Weak', 'None Detected'))
        } else {
            ...
            if (input$weak==0) {
                ...
                ct_value <- as.numeric(data$ct_value)
                cut <- cut(ct_value, 
                           c(input$verystrong,input$strong,input$moderate,input$weak,input$thr,100),
                           include.lowest = TRUE,
                           labels = c('Very Strong', 'Strong', 'Moderate', 'Weak', 'None Detected'))
            }
            cut
